...
public class TunablePIDController {
  ...
  /**
   * Create a TunablePIDController with the specified NT location and zeroed gains
   *
   * @param table The NT table to put the gains on
   * @param name The NT key to put the gains on
   */
  public TunablePIDController(String table, String name) {
    this.gains = new TunablePIDGains(table, name);
    this.pid = new PIDController(0.0, 0.0, 0.0, this.gains.period);
  }
   ...
   */
  public TunablePIDController(String table, String name, double kP, double kI, double kD) {
  ...
  public static class TunablePIDGains {
    ...
    public TunablePIDGains(
        String table, String name, double kP, double kI, double kD, double period) {
  ...
  public static class TunableProfiledPIDController {
    ...
    /**
     * Initialize controller with zeroed gains and constraints
     *
     * @param table The NT table to use
     * @param name The NT name to use
     */
    public TunableProfiledPIDController(String table, String name) {
      this(new TunablePIDGains(table, name), new TunableTrapezoidConstraints(table, name));
    }
    ...
    public TunableProfiledPIDController(
      ...
      this(
          new TunablePIDGains(table, name + " Gains", kP, kI, kD),
          new TunableTrapezoidConstraints(
              table, name + " Constraints", maxVelocity, maxAcceleration));
    }
