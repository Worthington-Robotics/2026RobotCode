...
public class GeomUtil {
   ...
   *
   * @param transform The original translation
   * @param bounds    The two corners of the bounding box to check for collision
   *                  with
   * @return The resulting translation
   */
   ...
   * @return The magnitude of the speeds vector. Does not factor in rotational
   *         velocity.
   */
  ...
  /**
   * Applies a ChassisSpeeds to a Pose2d to get what that pose will be after a
   * period of time
   *
   ...
   * @param driftRate The measured drift rate of the swerve drive, in radians per
   *                  second
  ...
  public static ChassisSpeeds driftCorrectChassisSpeeds(ChassisSpeeds speeds, double driftRate) {
    ...
    final Pose2d futurePose = new Pose2d(
    ...
    final ChassisSpeeds corrected = new ChassisSpeeds(
  ...
  public static double pointToLineSegmentDistance(
    ...
    final Translation2d projection = l1.plus(l2.minus(l1).times(t)); // Projection falls on the segment
   ...
   * @return The translated vector. If both vectors are at the same position,
   *         returns the same
   *         vector.
   */
  ...
  public static Pose2d fastExp(Pose2d pose, Twist2d twist) {
    ...
    final double translationX = transformX * poseRotation.getCos() - transformY * poseRotation.getSin();
    final double translationY = transformX * poseRotation.getSin() + transformY * poseRotation.getCos();

    final Translation2d newTranslation = new Translation2d(
  ...
  /**
   * Gets the distance between two points along some arbitrary axis by projecting
   * them onto the
  ...
  /**
   * Gets the distance between the robot and a target pose perpendicular to the
   * angle of the target
   *
  ...
  /***
   * Checks if three points are in counterclockwise order. Used for determining if
   * lines intersect.
   */
  ...
  public static boolean doLinesIntersect(Translation2d[] line1, Translation2d[] line2) {
    return (ccw(line1[0], line2[0], line2[1]) != ccw(line1[1], line2[0], line2[1]))
        && (ccw(line1[0], line1[1], line2[0]) != ccw(line1[0], line1[1], line2[1]));
  }

  /***
   * Checks if a line segment passes through an area.
   * 
   * @param line   A two item Translation2d array containing the two endpoints of
   *               the line segment to test.
   * @param bounds A two item Translation2d array containing the bounding points
   *               of the area.
  ...
  public static boolean doesLinePassThroughArea(Translation2d[] line, Translation2d[] bounds) {
    Translation2d[] testLine1 = { new Translation2d(bounds[0].getX(), bounds[0].getY()),
        new Translation2d(bounds[1].getX(), bounds[1].getY()) };
    Translation2d[] testLine2 = { new Translation2d(bounds[1].getX(), bounds[0].getY()),
        new Translation2d(bounds[0].getX(), bounds[1].getY()) };
   ...
   * 
   * @param line     A two item Translation2d array containing the two endpoints
   *                 of the line segment to test.
   * @param vertices A Translation2d array containing the cordinates of each
   *                 vertice of the polygon in sequential order. The order can be
   *                 CW or CCW.
  ...
  public static Translation2d clampTranslation(Translation2d pose) {
