...
public class AllianceFlipUtil {
    ...
    if (shouldFlip()) {
      return new Trajectory.State(
          ...
          new Pose2d(
              ...
              apply(state.poseMeters.getRotation())),
   ...
   */
  public static Pose2d addToFlipped(Pose2d flipped, double unflippedX, double unflippedY) {
    if (shouldFlip()) {
      unflippedX *= -1;
      unflippedY *= -1;
    }
    return new Pose2d(
        flipped.getX() + unflippedX, flipped.getY() + unflippedY, flipped.getRotation());
  }
