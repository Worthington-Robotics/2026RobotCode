...
public class PoseEstimator {
  ...
  private static final double HISTORY_LENGTH = 0.1;
  ...
  /** The amount of anti-jitter to apply. Lower values reduce the amount of jitter. */
  ...
  public void addDriveData(double timestamp, Twist2d twist) {
    addDriveDataNoUpdate(timestamp, twist);
    update();
  }

  /**
   * Records a new drive movement without updating
   *
   * @param timestamp The timestamp in seconds where the event occurred
   * @param twist The twist of the robot that represents the movement
   */
  public void addDriveDataNoUpdate(double timestamp, Twist2d twist) {
    ...
    for (TimestampedVisionUpdate timestampedVisionUpdate : visionData) {
      ...
      if (updates.containsKey(timestamp)) {
        ...
        if (prevUpdate == null || nextUpdate == null) {
          final var entry = updates.lastEntry();
          if (entry != null) {
            entry.getValue().visionUpdates.add(visionUpdate);
          }
  ...
  private static record PoseUpdate(Twist2d twist, ArrayList<VisionUpdate> visionUpdates) {
    public Pose2d apply(Pose2d lastPose, Matrix<N3, N1> q) {
      ...
      if (Math.abs(twist.dx) >= 0.00001
          || Math.abs(twist.dy) >= 0.00001
          || Math.abs(twist.dtheta) >= 0.0001) {
        pose = GeomUtil.fastExp(lastPose, twist);
      }
