...
public class GeneralMath {
  ...
  /**
   * Hard limits a velocity between two bounds. Essentially acts as virtual limit switches using an
   * encoder
   *
   * @param velocity The current velocity of the system
   * @param position The current position of the system
   * @param minPosition The minimum position of the system. Must be < max.
   * @param maxPosition The maximum position of the system. Must be > min.
   * @return The new velocity for the system, clamped if it goes too far.
   */
  public static double hardLimitVelocity(
      double velocity, double position, double minPosition, double maxPosition) {
  ...
  /**
   * Computes the absolute difference between two values in a wrapping range from 0-range. Note that
   * this always returns a positive answer, but it may be positive from the opposite direction
   *
  ...
  /**
   * Solves a 1D kinematic equation for time
   *
   * @param x_0 Initial position
   * @param x Final position
   * @param v_0 Inital velocity
   * @param a Acceleration
   * @return The positive solution for time
   */
  public static double solveKinematicTime(double x_0, double x, double v_0, double a) {
    final double dx = x - x_0;
    // Make sure we are accelerating the direction to go towards the target
    a = a * Math.signum(dx);
    final double sqrt = Math.sqrt(v_0 * v_0 - 2.0 * a * -dx);
    final double plus = (v_0 + sqrt) / a;
    final double minus = (v_0 - sqrt) / a;
    return Math.abs(Math.min(plus, minus));
    // return Math.sqrt(Math.abs((2 * (x - x_0)) / a));
  }

  public static double getOptimalRotationalPIDGoal(
      double current_position,
      double target_position,
      double current_velocity,
      double max_acceleration) {
    // Base case that just uses the same math from the continuous input inside the
    // WPILib PIDController
    if (Math.abs(current_velocity) < 0.0001) {
      // Get error which is the smallest distance between goal and measurement
      double errorBound = Math.PI;
      double goalMinDistance =
          MathUtil.inputModulus(target_position - current_position, -errorBound, errorBound);

      return goalMinDistance;
    }

    SmartDashboard.putNumber("Outputs/Velocity", Units.radiansToDegrees(current_velocity));
    SmartDashboard.putNumber(
        "Outputs/Target Position Unwrapped", Units.radiansToDegrees(target_position));
    current_position = MathUtil.inputModulus(current_position, 0.0, 2 * Math.PI);
    if (current_position == 2 * Math.PI) {
      current_position = 0.0;
    }
    target_position = MathUtil.inputModulus(target_position, 0.0, 2 * Math.PI);
    if (target_position == 2 * Math.PI) {
      target_position = 0.0;
    }
    SmartDashboard.putNumber("Outputs/Current Position", Units.radiansToDegrees(current_position));
    SmartDashboard.putNumber("Outputs/Target Position", Units.radiansToDegrees(target_position));

    double positivePosition = Math.abs(target_position - current_position);
    SmartDashboard.putNumber(
        "Outputs/Initial Positive Position", Units.radiansToDegrees(positivePosition));
    double negativePosition = 0.0;
    if (positivePosition < 0.0) {
      negativePosition = positivePosition;
      positivePosition = (2 * Math.PI) + positivePosition;
    } else {
      negativePosition = -((2 * Math.PI) - positivePosition);
    }

    SmartDashboard.putNumber("Outputs/Positive Position", Units.radiansToDegrees(positivePosition));
    SmartDashboard.putNumber("Outputs/Negative Position", Units.radiansToDegrees(negativePosition));

    final double positiveTime =
        solveKinematicTime(current_position, positivePosition, current_velocity, max_acceleration);
    final double negativeTime =
        solveKinematicTime(current_position, negativePosition, current_velocity, max_acceleration);

    SmartDashboard.putNumber("Outputs/Positive Time", positiveTime);
    SmartDashboard.putNumber("Outputs/Negative Time", negativeTime);

    if (positiveTime < negativeTime) {
